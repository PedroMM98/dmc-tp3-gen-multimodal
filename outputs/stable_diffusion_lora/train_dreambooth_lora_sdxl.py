...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if args.dataset_name is not None:
            ...
            # CAPTION_AWARE_PATCH: only load image files and attach same-stem .txt captions when present.
            image_extensions = {".png", ".jpg", ".jpeg", ".webp", ".jfif"}
            instance_image_paths = sorted(
                path for path in Path(instance_data_root).iterdir() if path.suffix.lower() in image_extensions
            )
            if not instance_image_paths:
                raise ValueError(f"No training images found in {instance_data_root}.")
            instance_images = [Image.open(path) for path in instance_image_paths]
            self.custom_instance_prompts = []
            for image_path in instance_image_paths:
                caption_path = image_path.with_suffix(".txt")
                caption_text = caption_path.read_text(encoding="utf-8").strip() if caption_path.exists() else ""
                self.custom_instance_prompts.extend(
                    itertools.repeat(caption_text if caption_text else self.instance_prompt, repeats)
                )
